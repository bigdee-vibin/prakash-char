...
─────────────────────────────────────────────────────────
  docs/*.md, ppt/*.pptx, docs/Asset_Register_v1.0.xlsx
          │
          ▼
  scripts/  (Python: Gemini API (Imagen), Firecrawl, Pillow/ImageMagick, ffmpeg, Pandoc)
...
├── prompts/                  # Gemini (Imagen) prompt library (one file per asset or per category)
...
```json
{
  ...
  "items": {
    ...
    "properties": {
      ...
      "source": { "enum": ["Gemini Images", "HTML/SVG", "SVG/HTML", "SVG", "HTML/CSS", "HTML", "CC0"] },
...
2. **Generate** imagery: `scripts/generate_images.py` reads `prompts/` (per-asset prompt + `prompts/style_lock.md` preamble), calls the Gemini API (Imagen), writes to `assets/images/` using the Asset Register's `File Name` convention.
