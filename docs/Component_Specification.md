...
**Behavior:** renders the shared "desk + idle terminal" background art (C-1), then — after a short beat, not instantly, so it doesn't feel like an error state — transitions the terminal's on-screen content to the Pause-for-Demo Interstitial (C-6a). Does **not** render any simulated tool output (Creative Bible §9, SRS FR-18) — this is a hard constraint on this component's implementation, not just a content-authoring choice. **Exception:** the OSINT Tool Chain terminal room (7b) additionally renders a short narrative content block (slide 13's "Where AI Fits In OSINT Now," DECISIONS.md #014) before the interstitial appears — an optional `content` array on that room's `terminal-room` scene entry (scene.schema.json), rendered the same way C-5 renders narrative content, immediately followed by C-6a.
